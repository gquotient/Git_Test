...
Strategy.reset = function(req, res, options, callback) {
  options = options || {};

  var reset_params = {};
  reset_params['pw'] = req.body.password;
  reset_params['pw_new'] = req.body.password_new;
  reset_params['client_id'] = options.clientId;
  reset_params['client_secret'] = options.clientSecret;
  ...
  var post_req = http.request(reset_post_options, function(post_res) {
    post_res.setEncoding('utf8');
      post_res.on('data', function (chunk) {
          ...
          callback( req, res, JSON.parse(chunk) );
      });
  });
